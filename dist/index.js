...
function getGitLog(repoPath, days, authorEmail) {
    const since = new Date(Date.now() - days * 86400000).toISOString();
    const authorFilter = authorEmail ? `--author="${authorEmail}"` : "";
    try {
        const raw = execSync(`git -C "${repoPath}" log --since="${since}" ${authorFilter} --format="%H|||%ae|||%ai|||%s" --numstat 2>/dev/null`, { encoding: "utf8", timeout: 10000 });
        const commits = [];
...
function detectGitRepos(startPath) {
    const repos = [];
...
server.tool("get_git_activity", "Read local git commit history across one or more repos for a time range. Returns structured commits with message, timestamp, files changed, and line stats. No API key required — reads local .git history only.", {
...
server.tool("get_github_activity", "Fetch PRs opened, merged, reviewed, and issues commented on GitHub in the last N days. Requires GITHUB_TOKEN env var with read:repo scope. Gracefully returns empty result if no token is configured.", {
    ...
    const since = new Date(Date.now() - days * 86400000).toISOString();
    try {
        const result = { prs_opened: [], prs_merged: [], prs_reviewed: [], issues_commented: [] };
        if (repos && repos.length > 0) {
            for (const repoSlug of repos) {
                ...
                for (const pr of prs) {
                    ...
                    if (pr.created_at > since)
                        result.prs_opened.push({ title: pr.title, url: pr.html_url, repo: repoSlug, state: pr.state, created_at: pr.created_at });
                    if (pr.merged_at && pr.merged_at > since)
                        result.prs_merged.push({ title: pr.title, url: pr.html_url, repo: repoSlug, merged_at: pr.merged_at });
                }
            }
        }
        else {
            ...
            for (const event of events) {
                ...
                if (event.type === "PullRequestEvent" && event.payload) {
                    ...
                    if (p.action === "opened")
                        result.prs_opened.push({ title: p.pull_request?.title, url: p.pull_request?.html_url, repo, created_at: event.created_at });
                    if (p.action === "closed" && p.pull_request?.merged)
                        result.prs_merged.push({ title: p.pull_request?.title, url: p.pull_request?.html_url, repo, merged_at: event.created_at });
                }
                if (event.type === "IssueCommentEvent") {
                    const p = event.payload;
                    result.issues_commented.push({ issue: p.issue?.title, url: p.issue?.html_url, repo: event.repo?.name, commented_at: event.created_at });
                }
            }
        }
...
server.tool("generate_standup", "Synthesize git commit history (and optionally GitHub activity) into a formatted daily standup update. Pass the raw output from get_git_activity (and optionally get_github_activity) and this tool formats it into a clean, ready-to-paste standup.", {
...
server.tool("generate_weekly_summary", "Generate a weekly engineering summary or freelancer client progress report from git and GitHub activity. Pass raw output from get_git_activity (with days=7) to produce a summary suitable for client reports, invoices, or team updates.", {
...
server.tool("list_configured_sources", "Show which activity sources are configured and ready to use. Checks for GITHUB_TOKEN, GITHUB_USERNAME, and local git repos. Use this to understand what data get_git_activity and get_github_activity will return.", {}, async () => {
